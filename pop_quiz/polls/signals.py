...
# before allowing a question to save
# check that it does not have a time conflict with the question before or after it
...
def pre_save_handler(sender, instance, *args, **kwargs):
    ...
    if (obj_before is not None and instance.starting_time <= obj_before.get_end_time()):
        raise Exception('clashed with object before')
    elif (obj_after is not None and instance.get_end_time() >= obj_after.starting_time):
        raise Exception('clashed with object after')
